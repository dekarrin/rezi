...
func Test_Enc_Slice_ValueIndirection(t *testing.T) {
	...
	t.Run("[]*struct, all non-nil", func(t *testing.T) {
		// setup
		assert := assert.New(t)
		var (
			input = []*testStructManyFields{
	...
	t.Run("[]*struct, one nil", func(t *testing.T) {
		// setup
		assert := assert.New(t)
		var (
			input = []*testStructManyFields{
				{Name: "KANAYA", Value: 8, Factor: 0.25, Enabled: true, hidden: make(chan int, 3), enabled: &sync.Mutex{}, inc: 48}, nil,
			}
	...
	t.Run("[]*struct, all nil", func(t *testing.T) {
		// setup
		assert := assert.New(t)
		var (
			input = []*testStructManyFields{nil, nil}
...
func Test_Dec_Slice_NoIndirection(t *testing.T) {
	...
	t.Run("[]struct", func(t *testing.T) {
		...
		// execute
		var actual []testStructManyFields
		consumed, err := Dec(input, &actual)
...
func Test_Dec_Slice_ValueIndirection(t *testing.T) {
	...
	t.Run("[]*struct, all nil", func(t *testing.T) {
		...
		var (
			...
			expect         = []*testStructManyFields{nil, nil}
			expectConsumed = 4
		)
