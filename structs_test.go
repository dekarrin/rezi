...
func Test_Dec_Struct(t *testing.T) {
	...
	runDecTests(t, "one-member struct", []byte{
		...
		0x41, 0x82, 0x05, 0x56, 0x61, 0x6c, 0x75, 0x65, // "Value"
		0x01, 0x04, // 4
	}, testStructOneMember{Value: 4}, 12)
	...
	runDecTests(t, "multi-member struct", []byte{
		...
		0x41, 0x82, 0x05, 0x56, 0x61, 0x6c, 0x75, 0x65, // "Value"
		0x01, 0x04, // 4
	}, testStructMultiMember{Value: 4, Name: "NEPETA"}, 28)
	...
	runDecTests(t, "with unexported", []byte{
		...
		0x41, 0x82, 0x05, 0x56, 0x61, 0x6c, 0x75, 0x65, // "Value"
		0x01, 0x04, // 4
	}, testStructWithUnexported{Value: 4, unexported: 0}, 12)
	...
	runDecTests(t, "with unexported case distinguished", []byte{
		...
		0x41, 0x82, 0x05, 0x56, 0x61, 0x6c, 0x75, 0x65, // "Value"
		0x01, 0x04, // 4
	}, testStructWithUnexportedCaseDistinguished{Value: 4, value: 0}, 12)

	runDecTests(t, "only unexported", []byte{0x00}, testStructOnlyUnexported{value: 0, name: ""}, 1)
...
func runDecTests[E any](t *testing.T, name string, filledInput []byte, filledExpect E, filledExpectConsumed int) {
	// normal value test
	t.Run(name, func(t *testing.T) {
		assert := assert.New(t)
